    ...
    public function submit() {
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            if (!empty($description) && !empty($name) && !empty($price) && !empty($stock) && !empty($category) && !empty($statut) && !empty($file_new)) {
                ...
                echo"<script>";
                echo"alert('produit ajoute avec succes');";
                echo" window.location.href = '/ecoservices/public/admin_produit';";
           echo" </script>";
                exit();
            } else {
    ...
    public function update() {
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            if ($this->validateData($name, $description, $price, $stock, $category, $statut, $file_new)) {
                ...
                echo"<script>
                alert('modification sauvegardee')
                window.location.href = '/ecoservices/public/admin_produit';
